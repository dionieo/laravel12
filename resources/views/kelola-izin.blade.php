...
<body>
    ...
    {{-- Alert Messages --}}
    @if (session('success'))
        <div class="alert success" style="margin: 1rem 1.5rem;">
    ...
    @if (session('error'))
        <div class="alert error" style="margin: 1rem 1.5rem;">
    ...
    <div class="summary-chips">
        ...
        <div class="chip approved">
            ...
            <div class="chip-content">
                <h3>{{ $historyList->where('status', 'approved')->count() }}</h3>
                <p>Disetujui</p>
            </div>
        </div>
        ...
        <div class="chip rejected">
            ...
            <div class="chip-content">
                <h3>{{ $historyList->where('status', 'rejected')->count() }}</h3>
                <p>Ditolak</p>
            </div>
        </div>
    </div>
    ...
    <div class="container">
            ...
            @foreach ($pendingList as $izin)
                @php
                    ...
                    if ($hoursPending < 1) {
                        ...
                        $pendingTimeText = floor($hoursPending / 24) . ' hari yang lalu';
                    }
                @endphp
                ...
                <div class="izin-card {{ $urgencyClass }}">
                    ...
                    <div class="card-header">
                        ...
                        <div class="student-info">
                            <div class="student-name">{{ $izin->nama_lengkap }}</div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="info-row">
                            ...
                            <div class="info-value">
                                {!! nl2br(e($izin->alasan_siswa)) !!}
                            </div>
                        </div>

                        @if ($izin->bukti_file)
                            <div class="info-row">
                                <a href="{{ $izin->bukti_file }}" target="_blank" class="attachment-link">
                        ...
                        <div class="info-row" style="margin-bottom: 0;">
                            ...
                            <div class="info-value" style="font-size: 0.875rem; color: #64748b;">
                                {{ \Carbon\Carbon::parse($izin->created_at)->format('d M Y, H:i') }} WIB
                            </div>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('kelola-izin.action') }}" class="admin-actions">
                        @csrf
        ...
        @if ($historyList->count() > 0)
            ...
            @foreach ($historyList->take(10) as $izin)
                @php
                    $initial = strtoupper(substr($izin->nama_lengkap, 0, 1));
                @endphp
                <div class="izin-card">
                    <div class="card-header">
                        ...
                        <div class="student-info">
                            <div class="student-name">{{ $izin->nama_lengkap }}</div>
                            <div class="student-meta">
                                <span>{{ \Carbon\Carbon::parse($izin->tanggal)->format('d M Y') }}</span>
                                <span class="meta-divider"></span>
                                <span>oleh {{ $izin->admin_name ?? 'Admin' }}</span>
                            </div>
                        </div>
                    ...
                    <div class="card-body">
                        <div class="info-row">
                            ...
                            <div class="info-value">{{ $izin->alasan_siswa }}</div>
                        </div>

                        @if ($izin->admin_notes)
                            <div class="info-row" style="margin-bottom: 0;">
                                <div class="info-label">Catatan Admin</div>
                                <div class="info-value">{!! nl2br(e($izin->admin_notes)) !!}</div>
                            </div>
                        @endif
                    </div>
                </div>
